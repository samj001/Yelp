...
def classification (category_name, train, label, list):
	...
	for review in train:
		...
		for token in review_list:
			if token in embedding.wv.vocab:
				min_distance = min(embedding.wv.distances(token,list))
				average_distance += min_distance
				length += 1

		average_distance = average_distance / length
		distance_feature.append(average_distance)

	X_train = sparse.hstack((X_train_tfidf, np.array(distance_feature)[:,None]))
...
classification("food", food_train,food_label,food_list)
classification("service", service_train,service_label,service_list)
