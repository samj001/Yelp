...
def extraction(category,train,l):
    indptr = [0]
    indices = []
    data = []
    vocabulary = {}
    ...
    for temp in train:
        ...
        for token in review_array:
            ...
            if token in model.wv.vocab:
                d_min = min(model.wv.distances(token, l))
                #sum[count] += d_min;
                if d_min > maxD[count]:
                    maxD[count] = d_min
                if d_min < minD[count]:
...
def classify(category,label, data_array):
    print("------------------------------------")
    ...
    scores = cross_validate(clf, data_array, label, scoring=scoring, cv=kf)
...
if __name__ == "__main__":
    dataFood = pandas.read_csv(r"balanced_food_training_data.csv")
    dataService = pandas.read_csv(r"balanced_service_training_data.csv")
    model = Word2Vec.load(r"food_service_embedding.model")
    ...
    food_bow = e.bag_words(dataFood['text'])
    food_tfidf = e.weight_by_tfidf(dataFood['text'])
    food_vector = extraction("food", dataFood['text'], food_list)
    
    service_tfidf = e.weight_by_tfidf(dataFood['text'])
    
    
    svm = classify("food",dataFood['label'], food_vector)
    
"""
review_text = load_review_text(5000,15000)
new_food_vector = extraction("food", review_text, food_list)
"""
