...
def classification(category,train, label, list):
    ...
    bi_clf = LinearSVC(C=1, max_iter=10000, dual=False, penalty='l2').fit(X_train_tfidf, label)
    bi_scores = cross_validate(bi_clf, X_train_tfidf, label, scoring=scoring, cv=kf, return_estimator=True)
    print("tf-idf: ",bi_scores['test_accuracy'].mean() )
    for temp in train:
        maxD.append(-1.0)
    ...
    #tf-idf tagged bigram
    # count_vect_bigram = CountVectorizer(ngram_range=(1,2))
    # X_train_counts_bigram = count_vect_bigram.fit_transform(train)
    # tfidf_transformer_bigram = TfidfTransformer()
    # X_train_tfidf_bigram = tfidf_transformer_bigram.fit_transform(X_train_counts_bigram)
    ...
    print(bi_scores['test_accuracy'].mean())
    index = 0
    # accuracy_best = []
    # accuracy_bi = []
    # for i in range (0, fold):
    #     accuracy_bi.append(bi_scores['test_accuracy'][i])
    #     accuracy_best.append(scores['test_accuracy'][i])

    ttest = scipy.stats.ttest_rel(bi_scores['test_accuracy'], scores['test_accuracy'])
    print(ttest)
    # print(accuracy_best,'\n')
    # print(accuracy_bi)
